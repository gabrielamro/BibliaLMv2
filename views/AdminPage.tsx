...
const SYSTEM_SOURCE_DOCS = {
    architecture: {
        ...
        content: `# 🏗️ BíbliaLM - Architecture & Technical Source of Truth\n\n> **AI INSTRUCTION:** Read this file FIRST before modifying any code. This defines the current project structure and patterns.\n> **VERSION:** v1.7.0 (Next App Router + Supabase baseline)\n\n## 1. Tech Stack (Current)\n*   **Core:** Next.js 16, React 18, TypeScript.\n*   **Styling:** TailwindCSS 4 + PostCSS.\n*   **Backend/DB:** Supabase (Auth, Postgres, Storage).\n*   **AI Engine:** Google GenAI SDK (\`@google/genai\`) via \`services/\`.\n*   **Icons:** Lucide React.\n*   **Routing:** Next App Router with compatibility helpers in \`utils/router.tsx\`.\n\n## 2. Directory Structure (Map)\n*   **\`/app\`**: Route entry points, layouts, API routes and page composition.\n*   **\`/components\`**: Reusable UI blocks.\n*   **\`/views\`**: Transitional page-level compositions still being absorbed into App Router.\n*   **\`/services\`**: External API logic, data access and integration boundaries.\n*   **\`/contexts\`**: Global state and app-wide providers.\n*   **\`/types.ts\`**: Single source of truth for shared TypeScript interfaces.\n\n## 3. Key Design Patterns\n### 3.1. Data Access (\`dbService\`)\n*   Components should not call Supabase directly.\n*   Use \`dbService\` methods from \`services/supabase.ts\`.\n\n### 3.2. AI Integration\n*   AI orchestration lives in \`services/\`.\n*   Feature access must be checked before paid flows.\n*   Outputs should remain structured and predictable: HTML for rich study content, JSON for typed payloads.\n\n### 3.3. Migration Status\n*   \`views/\` still exists for compatibility, but the architectural direction is to consolidate route ownership in \`app/\`.\n*   Legacy React Router and Firebase patterns are considered deprecated in the active app.`
    },
    context: {
        name: '_PROJECT_CONTEXT.md',
        content: `# 📖 BíbliaLM - Product Context & Business Rules\n\n> **AI INSTRUCTION:** This file contains the soul of the application.\n> **VERSION:** v1.7.0\n\n## 1. Product Vision\nUma plataforma profunda de estudo bíblico com IA, construída como um "Santuário": serena, acolhedora e centrada na Palavra.\n\n## 2. Core Modules\n### 2.1. Início (Home)\nDashboard principal com Pão Diário, progresso de leitura e atalhos.\n### 2.2. Bíblia (Reader)\nLeitura com foco, navegação por capítulos, áudio e ferramentas de estudo.\n### 2.3. O Reino (Social)\nFeed, igreja, célula, mural de oração e descoberta de conteúdo.\n### 2.4. Estúdio Criativo\nGeração de Artes Sacras e podcasts com IA.\n### 2.5. Workspace Pastoral\nAmbiente de liderança para jornadas, acompanhamento e conteúdo pastoral.\n\n## 3. Gamification\n*   **Currency:** Maná (XP).\n*   **Streaks:** uso consecutivo diário.\n*   **Badges:** ganhos por marcos e ações importantes.`
    },
    release: {
        name: '_RELEASENOTES.md',
        content: `# 📜 Histórico de Mudanças (Changelog)\n\n## [v1.7.0] - 2026-04-06 (Cleanup baseline)\n### Tipo: Architecture / Tooling\n- **Resumo:** limpeza de artefatos versionados por engano.\n- **Resumo:** restauração do typecheck com TypeScript cobrindo \`app/\`.\n- **Resumo:** remoção do último uso runtime de React Router.\n\n## [v1.6.x] - Base anterior\n### Tipo: App / Admin\n- **Resumo:** expansão do painel admin, integridade do sistema, onboarding e melhorias de conteúdo.\n\n## Nota\nO changelog completo oficial continua nos arquivos de documentação da raiz. Esta visão do admin é um resumo operacional.`
    },
    firestore: {
        name: 'supabase-notes.md',
        content: `# Supabase Notes\n\n## Current Reality\nO app ativo usa Supabase para autenticação, banco e storage.\n\n## Operational Rules\n*   Perfis vivem em \`profiles\`.\n*   Conteúdo social e estudos passam por \`dbService\`.\n*   Regras de acesso devem ser resolvidas por RLS e pela camada de serviços.\n\n## Legacy Notice\nArquivos como \`firestore.rules\` e \`storage.rules\` existem apenas como referência histórica de migração e não representam a arquitetura ativa.`
    },
    storage: {
        name: 'storage-notes.md',
        content: `# Storage Notes\n\n## Current Reality\nUploads do app ativo passam pelo Supabase Storage, normalmente no bucket \`uploads\`.\n\n## Recommended Pattern\n*   O cliente envia arquivos por helpers de serviço.\n*   URLs públicas e caminhos de upload devem ser centralizados na camada de serviços.\n*   O admin deve tratar regras de storage como documentação operacional, não como fonte primária de verdade do app.\n\n## Legacy Notice\nAs antigas regras do Firebase Storage permanecem apenas como registro histórico de uma stack anterior.`
    }
};
