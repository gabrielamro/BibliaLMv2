...
const SavedStudiesPage: React.FC = () => {
  ...
  const getSourceLabel = (source: string) => {
      switch(source) {
          case 'sermon': return 'Sermão';
          case 'devocional': return 'Devocional';
          case 'podcast': return 'Podcast';
          case 'modulo': return 'Trilha';
        ...
        <div className="max-w-6xl mx-auto pb-32">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 mb-6 no-scrollbar border-b border-gray-100 dark:border-gray-800">
                {[
                    ...
                    { id: 'notes', label: 'Anotações', icon: <BookOpen size={14}/> }
                ].map(t => (
                    <button 
                        key={t.id} 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in">
                    {filteredContent.map((item: any) => (
                        <StandardCard
                            ...
                            subtitle={activeTab === 'notes' ? item.content : (item.sourceText || 'Sem descrição')}
                            badges={activeTab !== 'notes' ? [
                                { label: getSourceLabel(item.source), color: 'bg-blue-50 text-blue-600' },
                                { label: item.status === 'published' ? 'Público' : 'Rascunho', color: item.status === 'published' ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500' }
