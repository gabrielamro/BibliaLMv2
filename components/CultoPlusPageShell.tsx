...
export default function CultoPlusPageShell({ children, isPastor = false, userName, avatar, compactDesktop = false }: CultoPlusPageShellProps) {
  ...
  const activeModule = useMemo(() => {
    const routeModule = getAppModuleForRoute(location.pathname, location.search);
    return visibleModules.find((item) => item.module === routeModule)
      ?? visibleModules.find((item) => isModuleActive(item, location.pathname))
      ?? visibleModules[0];
  }, [location.pathname, location.search, visibleModules]);
    ...
    <div data-testid={isFocusMode ? undefined : "cultoplus-page-shell"} data-module={activeModule.module} className="min-h-full bg-[#fdfbf7] text-[#2d2a26] dark:bg-[#0b0b0c] dark:text-gray-100">
      <div className="flex min-h-full items-stretch">
        <aside data-testid="cultoplus-desktop-menu" data-compact={isDesktopMenuCompact ? "true" : "false"} className={`sticky top-0 hidden h-screen shrink-0 flex-col border-r border-[#e6e0d8] bg-white py-4 transition-[width,padding] duration-200 lg:flex dark:border-white/10 dark:bg-[#111113] ${isFocusMode ? '!hidden' : ''} ${isDesktopMenuCompact ? 'w-[84px] items-center px-2' : 'w-[256px] px-4 py-6'}`}>
          ...
          <nav id="cultoplus-desktop-navigation" aria-label="Navegação principal da visão pessoal" className={`min-h-0 flex-1 space-y-1 overflow-y-auto no-scrollbar ${isDesktopMenuCompact ? 'mt-5 w-full' : 'mt-4 pr-1'}`}>
            {visibleModules.map((item) => {
              ...
              const isActive = item.module === activeModule.module;
              return <div key={item.label} data-module-theme={item.module} className="rounded-xl">
                <div className={`flex items-center rounded-xl transition ${isDesktopMenuCompact ? (isActive ? "module-nav-active ring-1 ring-[var(--module-border)]" : "module-nav-link") : `border-l-4 ${isActive ? "module-nav-active" : "module-nav-link border-transparent"}`}`}>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <main id="main-content" className="cultoplus-page-content flex flex-1 flex-col" tabIndex={-1}>{children}</main>
        </div>
      </div>
    </div>
  );
