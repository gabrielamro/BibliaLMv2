...
import { BookOpen, CalendarDays, ChevronRight, Church, FolderOpen, Home, Menu, PanelLeftClose, PanelLeftOpen, PenLine, Plus, Settings, Users } from "lucide-react";
...
export default function PastoralWorkspaceShell({ children }: { children: React.ReactNode }) {
  ...
  return <div data-module="pastoral" className="min-h-full bg-[#fdfbf7] text-[#2d2a26] dark:bg-[#0b0b0c] dark:text-gray-100"><div className="flex min-h-full"><aside data-compact={compact} className={`sticky top-0 hidden h-screen shrink-0 flex-col border-r border-[#e6e0d8] bg-white py-6 transition-[width,padding] duration-200 lg:flex dark:border-white/10 dark:bg-[#111113] ${compact ? "w-[76px] px-2" : "w-[272px] px-4"}`}><div className={`flex ${compact ? "flex-col items-center gap-2" : "items-center justify-between gap-2"}`}><CultoPlusBrand compact={compact} className={compact ? "!h-11 !w-11" : "min-w-0"} /><button type="button" onClick={toggleCompact} aria-expanded={!compact} aria-label={compact ? "Expandir menu" : "Esconder menu"} title={compact ? "Expandir menu" : "Esconder menu"} className="module-focus module-nav-link flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-gray-400">{compact ? <PanelLeftOpen size={15} /> : <PanelLeftClose size={15} />}</button></div>{!compact ? <><div className="mt-6"><AppViewSwitcher activeView="pastoral" canOpenPastoral canOpenManagement /></div><div className="module-gradient mt-4 rounded-2xl p-4 shadow-lg shadow-purple-950/10"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/70">Visão atual</p><h2 className="mt-2 text-lg font-black">Workspace Pastoral</h2><p className="mt-1 text-xs leading-5 text-white/75">Ensino, cuidado, salas e conteúdo.</p></div></> : null}<div className="mt-4 min-h-0 flex-1 overflow-y-auto no-scrollbar"><PastoralMenu compact={compact} /></div><Link href="/perfil" title={compact ? name : undefined} className={`module-focus module-nav-link mt-4 flex items-center rounded-xl border-t border-[#ece6df] p-2 pt-4 dark:border-white/10 ${compact ? "justify-center" : "gap-3"}`}>{avatar ? <img src={avatar} alt={name} className="h-10 w-10 rounded-full object-cover" /> : <span className="module-icon flex h-10 w-10 items-center justify-center rounded-full text-xs font-black">{name.slice(0, 2).toUpperCase()}</span>}{!compact ? <span className="min-w-0"><strong className="block truncate text-sm">{name}</strong><small className="text-xs text-gray-500">Visão pastoral</small></span> : null}</Link></aside><div className="min-w-0 flex-1"><header className="border-b border-[#e6e0d8] bg-white px-4 py-3 lg:hidden dark:border-white/10 dark:bg-[#111113]"><div className="flex items-center justify-between"><CultoPlusBrand /><details className="relative"><summary aria-label="Abrir menu do Workspace Pastoral" className="module-focus module-accent-bg flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-xl"><Menu size={20} /></summary><div className="absolute right-0 top-12 z-50 max-h-[78vh] w-[min(94vw,390px)] overflow-y-auto rounded-2xl border border-[#e6e0d8] bg-white shadow-2xl dark:border-white/10 dark:bg-[#151515]"><div className="border-b border-[#ece6df] p-4 dark:border-white/10"><strong className="block">Workspace Pastoral</strong><small className="text-gray-500">Ensino, cuidado e conteúdo</small></div><div className="border-b border-[#ece6df] p-3 dark:border-white/10"><AppViewSwitcher activeView="pastoral" canOpenPastoral canOpenManagement compact /></div><PastoralMenu mobile /></div></details></div></header>{children}</div></div></div>;
}
