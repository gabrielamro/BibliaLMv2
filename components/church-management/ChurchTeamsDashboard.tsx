...
export default function ChurchTeamsDashboard() {
  ...
  const createTeam = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!activeChurchId) {
      setCreateFeedback("Vincule seu perfil a uma igreja para criar equipes.");
      return;
    }
    if (!draft.name.trim() || !draft.area.trim()) {
      setCreateFeedback("Informe nome e area da equipe.");
      return;
    }
    ...
    try {
      ...
      setDraft({ name: "", area: "", description: "", capacity: "8", leaderId: "", functionsText: "" });
      setCreateModalOpen(false);
      await loadTeams();
    } catch (createError) {
      setCreateFeedback(createError instanceof Error ? createError.message : "Nao foi possivel criar a equipe.");
    } finally {
      setIsCreating(false);
    }
  };
    ...
    <main className="min-h-screen bg-[#f8fafc] text-[#071735]">
      ...
      {createModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4 py-6">
          <section role="dialog" aria-modal="true" aria-labelledby="new-team-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-5">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-500">Gestao da Igreja</p>
                <h2 id="new-team-title" className="mt-2 text-2xl font-black text-[#071735]">Nova Equipe</h2>
              </div>
              <button type="button" onClick={() => setCreateModalOpen(false)} aria-label="Fechar cadastro de equipe" className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 text-[#071735] transition hover:bg-slate-50">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={createTeam} className="p-5">
              ...
              <label className="mt-4 block">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Descricao</span>
                <textarea value={draft.description} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} rows={4} className="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-3 text-sm font-semibold leading-6 text-slate-900 outline-none transition focus:border-[#061b49]" />
              </label>

              <label className="mt-4 block">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Funcoes esperadas</span>
                <textarea
                  value={draft.functionsText}
                  onChange={(event) => setDraft((current) => ({ ...current, functionsText: event.target.value }))}
                  rows={4}
                  placeholder={"Uma por linha. Ex.:\nRecepcao - 4 voluntarios\nApoio - 2 voluntarios"}
                  className="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-3 text-sm font-semibold leading-6 text-slate-900 outline-none transition focus:border-[#061b49]"
                />
              </label>

              {createFeedback ? <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">{createFeedback}</p> : null}

              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:justify-end">
                <button type="button" onClick={() => setCreateModalOpen(false)} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-200 px-5 text-sm font-black text-[#061b49] transition hover:bg-slate-50">
                  Cancelar
                </button>
