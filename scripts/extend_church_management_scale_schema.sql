-- Extensao operacional legada da Gestao da Igreja.
-- A migracao canonica esta em supabase/migrations/20260723195503_complete_church_operations_and_quiz_security.sql.
-- Este arquivo permanece compativel para instalacoes que ainda usam os scripts diretos.
...
create table if not exists public.church_participation_logs (
  id uuid primary key default gen_random_uuid(),
  church_id uuid not null references public.churches(id) on delete cascade,
  service_id text null references public.church_services(id) on delete set null,
  team_id uuid null references public.church_service_teams(id) on delete set null,
  assignment_id uuid null references public.church_assignments(id) on delete set null,
