...
import { buildCaptureEntries, discoverAppRoutes } from './lib/routeCatalog.mjs';
...
const BASE_URL = process.env.CATALOG_BASE_URL ?? 'http://localhost:3010';
...
async function collectPageFiles(dir, acc = []) {
...
async function attemptLogin(page) {
  try {
    await page.goto(`${BASE_URL}/intro`, { waitUntil: 'domcontentloaded', timeout: 30000 });
...
async function captureRoute(page, entry, sessionLabel) {
  const targetUrl = `${BASE_URL}${entry.materializedRoute}`;
  const capture = {
    ...entry,
    sessionLabel,
    targetUrl,
...
function buildMarkdownReport(results, loginResult) {
...
async function main() {
  ...
  for (const entry of captureEntries) {
    const shouldUseAuth = entry.requiresAuth && loginResult.success;
    const page = shouldUseAuth ? authPage : publicPage;
    const sessionLabel = shouldUseAuth ? 'auth' : 'public';
    const result = await captureRoute(page, entry, sessionLabel);
    results.push(result);
    process.stdout.write(`${result.ok ? 'OK   ' : 'FAIL '} ${entry.materializedRoute}\n`);
  }
